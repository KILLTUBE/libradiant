...
class rectangle_t { public:
	...
	rectangle_t( float _x, float _y, float _w, float _h ) : x( _x ), y( _y ), w( _w ), h( _h ) {}
};

class XORRectangle { public:
	rectangle_t m_rectangle;
	GtkWidget* m_widget;
	GdkGC* m_gc;

	XORRectangle( GtkWidget* widget ) : m_widget( widget ), m_gc( NULL ) { }
...
class CamWnd : public GLWindow { public:
	...
	void ProjectCamera( const vec3_t A, vec_t B[2] ); // project a point in geometric space into camera space
};
