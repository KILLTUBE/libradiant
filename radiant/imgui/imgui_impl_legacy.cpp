...
void ImGui_ImplSdlGL2_RenderDrawData(ImDrawData* draw_data)
{
    ...
    qglPopMatrix();
    qglPopAttrib();
    qglPolygonMode(GL_FRONT, (GLenum)last_polygon_mode[0]);
	qglPolygonMode(GL_BACK, (GLenum)last_polygon_mode[1]);
...
bool ImGui_CreateDeviceObjects()
{
    ...
    qglTexImage2D(GL_TEXTURE_2D, 0, GL_ALPHA, width, height, 0, GL_ALPHA, GL_UNSIGNED_BYTE, pixels);
...
//
	...
	io.RenderDrawListsFn = ImGui_ImplSdlGL2_RenderDrawData;
    return true;
}
...
bool ImGui_NewFrame(/*SDL_Window *window*/)
{
    if (!g_FontTexture)
